...
const Maps = () => {
  ...
  useEffect(() => {
    ...
    //   .withUrl("https://localhost:7241/mapHub")
    //   .build();
    // setConnection(newConnection);
    const newConnection = new SignalR.HubConnectionBuilder()
      .withUrl("https://localhost:7241/mapHub")
      .withAutomaticReconnect()
      .configureLogging(SignalR.LogLevel.Information) // Access LogLevel here
      .build();
    ...
    newConnection.on("ReceiveLocationUpdate", (latitude, longitude) => {
      setLatitude(latitude);
      setLongitude(longitude);
    });
          ...
          {latitude && longitude && (
            <DirectionsService
              options={{
                ...
                destination: { lat: 25.59409, lng: 85.13756 },
                travelMode: "DRIVING",
              }}
              callback={handleDirectionsCallback}
            />
